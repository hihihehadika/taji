...
// Seluruh implementasi TPM hanya tersedia di platform non-WASM karena
// bergantung pada sistem berkas OS dan permintaan HTTP blocking.
...
mod implementasi {
    ...
    pub fn pasang_modul(url: &str) -> Result<PathBuf, String> {
        ...
        let direktori_tujuan = Path::new(super::DIREKTORI_MODUL);
        if !direktori_tujuan.exists() {
            fs::create_dir_all(direktori_tujuan).map_err(|e| {
                format!(
                    "tpm: gagal membuat direktori '{}': {}",
                    super::DIREKTORI_MODUL,
                    e
                )
            })?;
        }
    ...
    pub fn tampilkan_bantuan_tpm() {
        ...
        eprintln!();
        eprintln!(
            "Modul yang dipasang disimpan di folder '{}/'\ndan dapat digunakan via: masukkan(\"matematika\")",
            super::DIREKTORI_MODUL
        );
    }
}

// Re-ekspor fungsi publik agar pemanggil tidak perlu tahu soal modul internal.
#[cfg(not(target_arch = "wasm32"))]
pub use implementasi::{pasang_modul, tampilkan_bantuan_tpm};
